...
import { knowledgeSections, countTotalLinks, searchKnowledge } from "@/data/knowledge-base";
...
export function Dashboard() {
  ...
  const [activeSectionId, setActiveSectionId] = useState<string>(
    knowledgeSections[0]?.id ?? ""
  );

  // 검색어 상태
  const [searchQuery, setSearchQuery] = useState<string>("");

  // 전체 문서 수 (정적 계산 - 데이터 변경 시에만 재계산)
  const totalDocuments = useMemo(
    () => countTotalLinks(knowledgeSections),
    []
  );

  // 검색어에 따라 필터링된 섹션 데이터
  const filteredSections = useMemo(
    () => searchKnowledge(knowledgeSections, searchQuery),
    [searchQuery]
  );
