...
- **next-themes** (다크모드)
- **lucide-react** (아이콘)
- **Jest + React Testing Library** (테스트 333개, 브랜치 커버리지 92%+, 90% 이상 강제)
...
**단순 fetch 위젯 (StockWidget 등)**: `useFetchWidget` 훅으로 data/isLoading/error/retry 상태를 추상화하여 DRY 원칙을 준수한다.
...
[1단계] build-and-test (병렬 불가, 순차 실행)
  ...
  ├─ test:coverage                            ← 333개 테스트 + 커버리지 90%+
  └─ build                                    ← 프로덕션 빌드 성공 여부
  │
  ▼ (1단계 통과 시)
[2단계] e2e
