...
유팜시스템 기술 지원 지식베이스를 트렌디한 대시보드 스타일로 시각화하는 Next.js 16 앱.
...
- **Notion 공식 API 대신 `loadPageChunk` 사용 (의도적 선택)**: 공식 API와 동일 목적을 달성하지만 API 키 발급 + Notion integration 생성 + 대상 페이지 공유 설정 3단계가 추가된다. 사내 Notion 페이지가 이미 공개 상태이므로 인증이 불필요하며, API 키 없이 배포·유지보수가 가능한 것이 핵심 가치. **위험 수용 근거**: Notion 내부 스펙 변경 시 `notion-structure.ts` + `api/notion/[pageId]/route.ts` 2파일만 교체하면 공식 API로 전환 가능(마이그레이션 비용 낮음). 장애 시 `knowledge-base.ts` 정적 폴백으로 기본 탐색을 보장하므로 전면 중단 없음. **Rate-limit 위험**: Vercel Edge Cache(`s-maxage=300`)로 동일 페이지 반복 요청을 서버단에서 차단하여 Notion 측 IP 차단 위험을 최소화. 서버 프록시 구조이므로 클라이언트 IP가 Notion에 직접 노출되지 않음. 다수 동시 접속(>100 req/min)이 예상되면 Redis 캐시 레이어 추가를 권장.
...
- **드래그앤드롭 이중 구현**: 마우스는 HTML5 DnD API, 터치는 Pointer Events API + `setPointerCapture`로 별도 구현 (HTML5 DnD는 터치에서 불안정). 전체 DnD 로직은 `useDragDropWidgets.ts` 훅으로 분리되어 있다
- **지식베이스 localStorage 캐시**: `useKnowledgeStructure` 훅이 fetch 성공 시 `upharm_knowledge_structure` 키로 캐시 저장. fetch 실패 시 캐시 폴백(`cached` 상태)으로 빈 화면 방지. SSR 하이드레이션 불일치를 막기 위해 `useState` 초기값은 항상 `[]`, 캐시 로딩은 `useEffect` 내 `load()` 에서만 처리
...
- **next-themes** (다크모드)
- **lucide-react** (아이콘)
- **Jest + React Testing Library** (테스트 389개, 브랜치 커버리지 90%+, 90% 이상 강제)
- **Playwright** (E2E 4종: smoke·widgets·section·viewport)
...
src/app/page.tsx              ← Async Server Component: Notion에서 구조 로딩
       └─ Dashboard.tsx       ← Client Component: 상태 관리 (활성 섹션, 검색어)
                               useKnowledgeStructure 훅으로 클라이언트 캐시 폴백 관리
           ...
           ├─ HomeView.tsx    ← 홈 위젯 렌더러 + useDragDropWidgets 훅 연동
           │   └─ widgets/   ← 위젯 17파일 (WidgetCard + WidgetFeedback + 14종 위젯 + index.ts 배럴)
           └─ SectionView.tsx ← 카테고리 카드 그리드
               └─ CategoryCard.tsx ← 카드 (더 보기/접기, 모달 연동, 열람 수 기록)
                   └─ NotionModal.tsx      ← 모달 UI + 페이지 내비게이션
                       └─ NotionBlockRenderer.tsx ← 블록 타입별 JSX 렌더러 (분리됨)

src/app/api/notion/[pageId]/route.ts      ← Notion loadPageChunk 래핑
src/app/api/knowledge-structure/route.ts  ← 지식베이스 구조 조회 API Route (클라이언트 폴백용)
src/app/api/market/route.ts              ← Yahoo Finance 프록시 (CORS 우회, 60초 캐시)
src/lib/notion-structure.ts              ← Notion 3단계 계층 파싱 (서버 전용)
src/lib/notion-converter.ts              ← loadPageChunk → 공식 API 형식 변환 유틸 (route에서 분리)
...
src/hooks/useKnowledgeStructure.ts       ← 지식베이스 로딩 훅 (loading→retrying→success|cached|error 상태 머신, localStorage 캐시)
src/hooks/useDragDropWidgets.ts          ← 위젯 DnD 상태·핸들러 훅 (HomeView에서 분리)
src/hooks/usePomodoro.ts                 ← 포모도로 타이머 커스텀 훅 (관심사 분리)
src/hooks/useFetchWidget.ts              ← 외부 API 위젯 공통 fetch 훅 (data/isLoading/error/retry, DRY)
src/hooks/useIMEInput.ts                 ← 한국어 IME 조합 추적 훅 (localValue, isComposingRef, Header/HomeView 공용)
src/components/widgets/index.ts          ← 위젯 14종 배럴 익스포트
src/components/widgets/WidgetCard.tsx    ← 공통 카드 래퍼 (상단 스트라이프·헤더·스크롤 영역)
src/components/widgets/WidgetFeedback.tsx ← 공통 로딩 스피너·에러 UI (WidgetLoadingSpinner, WidgetError)
src/components/widgets/*Widget.tsx       ← 위젯 14종 개별 파일 (각 200줄 미만)
```

### 홈 위젯 목록 (src/components/widgets/)

| 위젯 | 컴포넌트 | 데이터 소스 |
|------|----------|-------------|
| 최근 수정 Top 5 | `RecentlyModifiedWidget` | sections props |
...
모든 DnD 상태와 핸들러는 `src/hooks/useDragDropWidgets.ts`로 분리되어 있으며, `HomeView.tsx`는 훅 반환값을 그대로 사용한다.
...
- **hover lift 위치 주의**: `WidgetCard` 내부에 `hover:-translate-y-0.5` CSS transform을 적용하면 HTML5 DnD 고스트 캡처 및 `ondragover` 히트 테스트가 Chrome/Edge에서 오작동한다. hover lift는 반드시 `draggable` 래퍼에 `!draggingId && "hover:-translate-y-0.5"` 조건으로 적용할 것
...
클라이언트(폴백): Dashboard → useKnowledgeStructure()
       → /api/knowledge-structure → Notion loadPageChunk
       → 성공: localStorage 캐시 저장 + status="success"
       → 실패 + 캐시 있음: 캐시 표시 + status="cached" (KnowledgeCachedBanner)
       → 실패 + 캐시 없음: status="error" (재시도 UI)

클라이언트: CategoryCard 클릭
       → /api/notion/[pageId] → loadPageChunk → 블록 JSON → notion-converter.ts 변환
       → NotionModal → NotionBlockRenderer 렌더링 + recordPageView(localStorage)
...
│                                                                 │
│  useWidgetStore (upharm_widget_order)                           │
│    widgetOrder: WidgetId[]  ←→  reorderWidgets(from, to)       │
│         │                                                       │
│  useMemoStore (upharm_memos)                                    │
│    memos: Memo[]            ←→  addMemo / removeMemo           │
│         │                                                       │
│  useTodoStore (upharm_todos)                                    │
│    todos: Todo[]            ←→  addTodo / toggleTodo / remove  │
│         │                                                       │
│  useDDayStore (upharm_ddays)                                    │
│    ddays: DDay[]            ←→  addDDay / removeDDay           │
│         │                                                       │
│  useBookmarkStore (upharm_bookmarks)                            │
│    bookmarks: Bookmark[]    ←→  addBookmark / removeBookmark   │
└──────────────────────┬──────────────────────────────────────────┘
                       │ useXxxStore() 훅으로 구독
                       ▼
┌──────────────────────────────────────────────────────────────────┐
│                    HomeView.tsx (위젯 렌더러)                     │
│                                                                  │
│  widgetOrder → renderWidget(id) → 14개 위젯 컴포넌트             │
│                                                                  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐   │
│  │ MemoWidget   │  │ TodoWidget   │  │ PomodoroWidget       │   │
│  │ (useMemo     │  │ (useTodo     │  │ (usePomodoro hook    │   │
│  │  Store)      │  │  Store)      │  │  내부 상태만)         │   │
│  └──────────────┘  └──────────────┘  └──────────────────────┘   │
│  ┌──────────────┐  ┌─────────────────────────────────────────┐   │
│  │ DDayWidget   │  │ API 위젯 (외부 fetch, isLoading/error)  │   │
│  │ BookmarkWidget│  │ Weather/Exchange/Stock/Calendar/Weekly  │   │
│  └──────────────┘  └─────────────────────────────────────────┘   │
└──────────────────────────────────────────────────────────────────┘

DnD 흐름 (마우스: HTML5 / 터치: Pointer Events):
  핸들 드래그 시작 → draggingId 설정 → 드롭 대상 탐지
  → useWidgetStore.reorder(fromId, toId) → widgetOrder 업데이트
  → localStorage 자동 영속화 (Zustand persist)
```

## 위젯 비동기 fetch 패턴

`src/components/widgets/` 하위 외부 API 위젯(WeatherWidget, ExchangeRateWidget, StockWidget, CalendarWidget, WeeklyWeatherWidget)은 공통 패턴을 따른다.

**단순 fetch 위젯 (StockWidget, ExchangeRateWidget 등)**: `useFetchWidget` 훅으로 data/isLoading/error/retry 상태를 추상화하여 DRY 원칙을 준수한다.

```typescript
// useFetchWidget 훅 적용 — DRY 패턴
const { data, isLoading, error, retry } = useFetchWidget<T>(
  () => fetch(url).then(r => r.json()).then(transform)
);
...
**위젯 로딩·에러 UI 공통화**: `WidgetLoadingSpinner`와 `WidgetError`는 `src/components/widgets/WidgetFeedback.tsx`에 정의되어 있다. 새 위젯 추가 시 인라인 div 대신 이 컴포넌트를 사용한다.
...
- 테스트 커버리지 90% 이상 유지 (전역 기준: 현재 구문 96.86% / 브랜치 90.84% / 라인 98.08%)
...
- **영속 상태는 반드시 Zustand persist 스토어로 관리** — `localStorage` 직접 접근 금지 (SSR 하이드레이션 불일치 유발)
  ...
  - 예외: `useKnowledgeStructure`(지식베이스 구조 캐시) — SSR-safe를 위해 `useEffect` 내에서만 localStorage 접근, `useState` 초기값은 항상 `[]`
...
- `e2e/widgets.spec.ts` — 위젯 상호작용: 계산기(연산·초기화)·메모(추가·삭제)·투두(추가·완료·삭제)·포모도로(시작·일시정지·모드전환)·다크모드 토글·드래그 핸들 존재 확인
- `e2e/section.spec.ts` — 섹션 카드 링크 클릭 → NotionModal 열림/닫힘(X 버튼·ESC), 외부 링크 href 검증
- `e2e/viewport.spec.ts` — 반응형 레이아웃: 모바일(375px) 사이드바 숨김, 태블릿(768px)·데스크톱(1280px) 사이드바 표시 검증
...
[1단계] build-and-test (병렬 불가, 순차 실행)
  ...
  ├─ test:coverage                            ← 361개 테스트 + 커버리지 90%+
  └─ build                                    ← 프로덕션 빌드 성공 여부
  │
  ▼ (1단계 통과 시)
[2단계] e2e
  ...
  ├─ test:e2e (smoke·widgets·section·viewport 4종 스펙)
  │  실패 시 playwright-report 아티팩트 7일 보관
  ├─ Lighthouse CI (@lhci/cli autorun, .lighthouserc.json 기준)
  │  성능≥0.7 / 접근성·best-practices·SEO ≥0.8 (warn, 파이프라인 비차단)
  └─ lighthouse-results 아티팩트 7일 보관
  │
...
**3단계 자동화 추가 기능**:
- Vercel CLI 출력에서 `*.vercel.app` URL을 추출 → `actions/github-script`로 PR 코멘트에 클릭 가능한 프리뷰 URL 자동 게시
- 배포 후 HTTP 200 헬스체크(최대 5회·20초 간격)로 broken deploy 조기 감지

**4단계 자동화 추가 기능**:
- 프로덕션 URL 헬스체크 (5회 재시도)로 배포 성공 여부 자동 검증
- `$GITHUB_STEP_SUMMARY`에 배포 URL·커밋 SHA·롤백 방법 기록 → Actions Summary 탭에서 팀이 즉시 참조 가능
