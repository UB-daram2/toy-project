...
| **생산성 위젯** | 12종 (날씨·포모도로·D-Day·Todo·계산기·시계 등) | 없음 | 없음 | 없음 | 구현 비용 큼 |
...
**차별화 포인트**: Notion을 CMS로 유지하면서 API 키·서버·라이선스 비용 없이 10분 안에 배포 가능한 최소 비용 대시보드. 지식베이스 탐색 + 업무 생산성 도구(날씨·포모도로·D-Day)를 단일 화면에 통합. 기존 Notion 워크플로를 바꾸지 않고 **읽기 전용 레이어**로 추가하는 것이 핵심 가치.
...
- **지식베이스 로딩**: `useKnowledgeStructure` 훅이 `/api/knowledge-structure`를 호출하고, 실패 시 최대 2회 자동 재시도한다. 캐시 없이 항상 최신 데이터를 요청하며, 모든 시도 실패 시 에러 UI + Notion 페이지 안내 + 재시도 버튼을 표시한다
...
- **next-themes** (다크모드)
- **lucide-react** (아이콘)
- **Jest + React Testing Library** (테스트 249개, 브랜치 커버리지 80%+)
...
npm test              # 테스트 실행
npm run test:coverage # 커버리지 리포트 (80% 미달 시 실패)
...
src/app/page.tsx              ← Async Server Component: Notion에서 구조 로딩
       └─ Dashboard.tsx       ← Client Component: 상태 관리 (활성 섹션, 검색어)
                               useKnowledgeStructure 훅으로 클라이언트 폴백 관리
           ...
           │   └─ widgets/   ← 위젯 15파일 (WidgetCard + WidgetFeedback + 12종 위젯 + index.ts 배럴)
...
src/hooks/useKnowledgeStructure.ts       ← 지식베이스 로딩 훅 (loading→retrying→success|error 상태 머신)
...
src/hooks/useEasterEgg.ts                ← 이스터에그 훅 (N번 빠르게 클릭 시 모달 표시)
src/components/KnowledgeStatusUI.tsx     ← 지식베이스 로딩 상태별 UI (배너·로딩·에러 4종, Dashboard에서 분리)
src/components/widgets/index.ts          ← 위젯 12종 배럴 익스포트
src/components/widgets/WidgetCard.tsx    ← 공통 카드 래퍼 (상단 스트라이프·헤더·스크롤 영역)
src/components/widgets/WidgetFeedback.tsx ← 공통 위젯 UI (WidgetLoadingSpinner, WidgetError, WidgetEmptyState, FormActionButtons)
src/components/widgets/*Widget.tsx       ← 위젯 12종 개별 파일 (각 200줄 미만)
```
...
클라이언트(폴백): Dashboard → useKnowledgeStructure()
       ...
       → 성공: status="success"
       → 실패: status="error" (재시도 UI + Notion 안내)
...
│                    HomeView.tsx (위젯 렌더러)                     │
│                                                                  │
│  widgetOrder → renderWidget(id) → 12개 위젯 컴포넌트             │
│                                                                  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐   │
...
│  │ BookmarkWidget│  │ Weather/Calendar/Weekly                │   │
│  └──────────────┘  └─────────────────────────────────────────┘   │
└──────────────────────────────────────────────────────────────────┘
...
`src/components/widgets/` 하위 외부 API 위젯(WeatherWidget, CalendarWidget, WeeklyWeatherWidget)은 공통 패턴을 따른다.

**단순 fetch 위젯 (CalendarWidget 등)**: `useFetchWidget` 훅으로 data/isLoading/error/retry 상태를 추상화하여 DRY 원칙을 준수한다.
...
**위젯 공통 UI**: `WidgetLoadingSpinner`, `WidgetError`, `WidgetEmptyState`, `FormActionButtons`는 `src/components/widgets/WidgetFeedback.tsx`에 정의되어 있다. 새 위젯 추가 시 인라인 div 대신 이 컴포넌트를 사용한다.
...
| 전체 커버리지 | 80% 이상 강제 (jest.config.ts `coverageThreshold`) |
...
[1단계] build-and-test (병렬 불가, 순차 실행)
  ...
  ├─ test:coverage                            ← 249개 테스트 + 커버리지 80%+
  └─ build                                    ← 프로덕션 빌드 성공 여부
  │
  ▼ (1단계 통과 시)
[2단계] e2e
