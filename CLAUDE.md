...
- **next-themes** (다크모드)
- **lucide-react** (아이콘)
- **Jest + React Testing Library** (테스트 361개, 브랜치 커버리지 92%+, 90% 이상 강제)
...
src/app/page.tsx              ← Async Server Component: Notion에서 구조 로딩
       └─ Dashboard.tsx       ← Client Component: 상태 관리 (활성 섹션, 검색어)
           ...
           ├─ HomeView.tsx    ← 홈 위젯 렌더러 + 드래그앤드롭 재정렬 (위젯은 widgets/ 하위 분리)
           │   └─ widgets/   ← 위젯 16파일 (WidgetCard + 14종 위젯 + index.ts 배럴)
